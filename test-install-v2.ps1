...
.DESCRIPTION
    This script handles prerequisites, Git cloning (now as the final step), Python virtual environment creation using 'uv',
...
function Download-Requirements-Only {
    param(
        ...
        [string]$OdooVersion,
        [Parameter(Mandatory = $true)]
        [string]$CloneDir # This is the target directory for requirements.txt
    )
    ...
    # 3. Create mock directories for addons that the script expects to find later
...
try {
    ...
    # ----------------------------------------------------
    # LOGIC: Define and create the new parent folder
    # ----------------------------------------------------
    ...
    # 4. Generate Odoo.conf (needs mock dirs to be present)
    Set-Location $parentInstallDir
    Generate-Odoo-Conf -OdooVersion $selectedOdooVersion -BaseInstallPath $parentInstallDir -OdooCloneDir $cloneDir

    # 5. FINAL STEP: Perform the actual Git Clone (overwrites mock directories)
